...
export default function YouTubeSearch({ onSearch, isSearching, error }: YouTubeSearchProps) {
  ...
  return (
    <div className="relative min-h-screen bg-retro-bg flex flex-col items-center justify-center p-4 selection:bg-hot-pink selection:text-white">
      {/* Sparkles Floating */}
      {sparkles.map((s) => (
        <div
          key={s.id}
      ...
      <div className="relative max-w-lg w-full pixel-border-window p-6 md:p-8 flex flex-col items-center text-center z-10">
        ...
        <div className="mb-6 flex flex-col items-center justify-center">
          <div className="relative w-40 h-24 bg-hot-pink border-4 border-black box-content flex flex-col justify-between p-2 pixel-sprite-walk select-none shadow-[4px_4px_0_0_#C2185B]">
            <div className="w-full h-8 bg-bubblegum border-2 border-black flex justify-around items-center px-2">
              <div className="w-4 h-4 bg-[#250017] border-2 border-black rounded-full" />
            ...
            <div className="flex justify-between items-end w-full px-4 h-4">
              <div className="w-3 h-2 bg-[#250017] border-2 border-black border-b-0" />
              <div className="w-10 h-2 bg-bubblegum border-2 border-black border-b-0 flex justify-around items-center">
                <span className="w-1 h-1 bg-[#250017]" />
                <span className="w-1 h-1 bg-[#250017]" />
              </div>
              <div className="w-3 h-2 bg-[#250017] border-2 border-black border-b-0" />
            </div>
            <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 flex gap-4 mt-2">
              <div className="w-2 h-3 bg-white flex items-end">
                <div className="w-2 h-1 bg-[#250017]" />
              </div>
              <div className="w-2 h-3 bg-white flex items-end">
                <div className="w-2 h-1 bg-[#250017]" />
              </div>
          ...
          <div className="flex gap-12 -mt-1 select-none">
            <div className="w-4 h-3 bg-white border-2 border-black border-t-0 shadow-[2px_2px_0_0_#C2185B]" />
            <div className="w-4 h-3 bg-white border-2 border-black border-t-0 shadow-[2px_2px_0_0_#C2185B]" />
          </div>
        </div>
        ...
        <p className="text-[9px] md:text-[10px] text-gray-300 max-w-sm mt-4 mb-6 leading-relaxed uppercase">
        ...
        <form onSubmit={handleSubmit} className="w-full flex flex-col gap-4">
          <input
            ...
            className="w-full bg-retro-dark border-4 border-black p-4 text-white text-xs md:text-sm font-mono placeholder-gray-500 focus:outline-none focus:border-hot-pink transition-colors font-bold uppercase shadow-inner"
            required
            autoFocus
          />
          <button
        ...
        {/* Global Error Notice */}
        {error && (
          <div className="mt-4 w-full p-3 bg-retro-dark border-4 border-[#C2185B] text-hot-pink font-mono text-[9px] flex items-start gap-2 text-left shadow-md">
